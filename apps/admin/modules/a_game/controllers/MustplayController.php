...
class MustplayController extends BackendController
{
	...
	{	
        ...
        $search['pageSize']=15;
        $datalist['result']=MygameService::getGameMustPlayList('android',$search['page'],$search['pageSize']);
        if(!empty($datalist))
        {
        	foreach ($datalist['result'] as $key=>&$value) {
        		$arr=GameService::getMultiInfoById(array('0'=>$value['agid']),'android');
        		$value['gname']=!empty($arr)?$arr['0']['shortgname']:'';
        	}
        }
        $datalist['totalCount']=MygameService::getGameMustPlayCount('android');
