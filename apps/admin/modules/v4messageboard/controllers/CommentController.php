<?php
namespace modules\v4messageboard\controllers;
...
use modules\v4messageboard\models\Comment;
...
class CommentController extends BackendController
{
    ...
    {
        $this->current_module = 'v4messageboard';
    }
...
//         $uid = parent::getSessionUserUid();
    ...
    public function getExcel (){
        ...
        $excel->getActiveSheet()->setTitle('留言板');
        ...
        header('Content-Disposition: attachment;filename="'. '留言板' .date('Y-m-d').'.xlsx"');
...
//        print_r($input);die;
        ...
        if($res && !$res['errorCode']){
            ...
            return $this->redirect('v4messageboard/comment/index','成功');
        }else{
            return $this->redirect('v4messageboard/comment/index',$res['errorDescription']);
        }
    }
    
    public function getSubCommentList($rid=''){
        if(!$rid) return $this->redirect('/v4messageboard/comment/index')->with('global_tips','数据错误');
        $page = Input::get('page',1);
        $limit = 10;
...
//                print_r($input);exit;
        ...
        if($res && !$res['errorCode']){
            return $this->redirect('v4messageboard/comment/sub-comment-list/'.$input['replyId'],'成功');
        }else{
            return $this->redirect('v4messageboard/comment/sub-comment-list/'.$input['replyId'],$res['errorDescription']);
        }
    }
