...
class PriceController extends BackendController
{
    ...
    public function getList()
    {
        ...
        $res = AllService::excute2("jinyu", $search, "jinyu_vote/search/search_price_info");
...
//            $total = $res['totleCount'];
    ...
    public function getAdd()
    {
        ...
        if($input['priceId']){
            $res = AllService::excute2("jinyu",$input,"jinyu_vote/search/search_price_info");
    ...
    public function postAdd()
    {
        ...
        if(Input::file('pricePic')){
            ...
            if ($input['price_img']) {
                $input['pricePic'] = $input['price_img'];
            }
        }
        if($input['priceId']){
            $res= AllService::excute2("jinyu",$input,"jinyu_vote/update/update_priceInfo",false);
        }else{
            $res= AllService::excute2("jinyu",$input,"jinyu_vote/add/add_priceInfo",false);
        }
        if($res['success']){
            if ($input['priceId']) {
                return $this->redirect('jinyu/price/list','修改成功');
            } else {
                return $this->redirect('jinyu/price/list','添加成功');
            }
        }else{
