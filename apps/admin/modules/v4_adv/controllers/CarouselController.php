...
class CarouselController extends \Youxiduo\MyService\SuperController
{
    const  Adv_logo='首页轮播位';//;
...
//         }
    ...
    private function getInfo($inputinfo){
        ...
        if(Input::hasFile('adv_img')){
            $inputinfo['adv_img']=MyHelp::save_img(Input::file('adv_img'));
        }else{
            unset($inputinfo['adv_img']);
        }
        $inputinfo['adv_logo']=self::Adv_logo;
        $keys=array_flip(explode(',',self::Three));
        $inputinfo['thirdParty']=MyHelp::setJson($inputinfo,$keys);
        ...
        if($inputinfo['attribute'] == 2 and ($this->_config['date'] >= $inputinfo['startTime'])){
            $inputinfo['is_show']=1;
        }
        if($inputinfo['attribute'] == 3 and ($this->_config['date'] >= $inputinfo['endTime'])){
            $inputinfo['is_show']=0;
        }
        return $inputinfo;
        if(Input::hasFile('adv_img')){
