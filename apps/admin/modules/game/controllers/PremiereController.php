...
class PremiereController extends BackendController{
    ...
    public function postAdd(){
        $input = Input::all();
        $rule = array('title'=>'required','gid'=>'required','state'=>'required','addtime'=>'required');
        $prompt = array('title.required'=>'请填写标识名称','gid.required'=>'请选择游戏','state.required'=>'请填写游戏状态','addtime.required'=>'请选择开测时间');
        ...
        if($valid->fails()){
            ...
            if(GameFirst::add($data)){
                return $this->redirect('/game/premiere/list','添加成功');
            }else{
    ...
    public function getEdit($id){
        ...
        $game = array();
        if(isset($info['gid'])){
            $game = GameService::getOneInfoById($info['gid'],'ios');
        }
    ...
    public function postEdit(){
        $input = Input::all();
        $rule = array('id'=>'required','title'=>'required','gid'=>'required','state'=>'required','addtime'=>'required');
        $prompt = array('id.required'=>'数据错误','title.required'=>'请填写标识名称','gid.required'=>'请选择游戏','state.required'=>'请填写游戏状态','addtime.required'=>'请选择开测时间');
        $valid = Validator::make($input,$rule,$prompt);
        if($valid->fails()){
            return $this->back()->withInput()->with('global_tips',$valid->messages()->first());
        }else{
            ...
            if(GameFirst::update($input['id'],$data)){
                return $this->redirect('/game/premiere/list','保存成功');
            }else{
