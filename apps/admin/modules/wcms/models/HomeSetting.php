...
class HomeSetting extends BaseHttp
{
	 ...
	 * @return array
	 */
	public static function GetIndexShanPing()
	{
		$api_url = self::HOST_URL . 'GetIndexShanPing';
		$params = array();

		$result = self::http($api_url, $params);
		//print_r($result);exit;
		if ($result !== false && $result['errorCode'] == 0) {
			return isset($result['result'][0]) ? $result['result'][0] : array();
		}
		return array();
	}
	 ...
	 * @return bool
	 */
	public static function SaveIndexShanPing($type,$flashUrl,$sikpUrl,$autoClose,$onOff,$closeTime)
	{
		$api_url = self::HOST_URL . 'SaveIndexShanPing';
		$params = array();

		$params['type'] = $type;
		$params['flashUrl'] = $flashUrl;
		$params['skipUrl'] = $sikpUrl;
		$params['autoClose'] = $autoClose ? "true" : "false";
		$params['onOff'] = $onOff;
