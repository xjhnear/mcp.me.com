...
class BbsController extends BackendController{
    ...
    public function getForumList(){
        ...
        $limit = 20;
        $search_name = Input::get('fname','');
        $bbs = TopicServiceT::getForums($search_name,$page,$limit);

        if($bbs['errorCode']) return $this->back()->with('global_tips','获取论坛列表接口错误，请联系管理员');
        $bbs = $bbs['result'];
        $bbs_count = TopicServiceT::getForumsCount($search_name);
        ...
        if($bbs){
            ...
            $relat_result = TopicServiceT::getForumAndGameRelation(1,1,$fids);
            ...
            if($gids){
                ...
                $people_result = TopicServiceT::getForumPeopleNum('ios',implode(',',$gids));
    ...
    public function postAddForum(){
        ...
        if($valid->fails()){
            ...
            $platform = 1;
            //新建论坛
            $add_forum_result = TopicServiceT::addForum($input['forum_name'],$input['gicon']);
            if($add_forum_result['errorCode'] || !$add_forum_result['result']) return $this->back()->with('global_tips','添加论坛失败，请重试');
            //保存论坛游戏关系
            $add_relate_result = TopicServiceT::saveForumAndGameRelation($add_forum_result['result'],$input['gid'],$platform);
    ...
    public function postAddHotlistDo(){
        ...
        if($valid->fails()){
            ...
            $gid = Input::get('gid');
            $relate = TopicServiceT::getForumAndGameRelation(1,1,false,$gid);
    ...
    public function getCheckGameHasBbs(){
        ...
        if($gameid && is_numeric($gameid)){
            $relate = TopicServiceT::getForumAndGameRelation(1,1,false,$gameid);
