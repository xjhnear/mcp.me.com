<?php
namespace modules\webt_forum\controllers;
...
use Youxiduo\Bbs\TopicServiceT;
...
class TopicController extends BackendController
{
    ...
    {
        $this->current_module = 'webt_forum';
        $board_result = TopicServiceT::getBoardList('','',null,null,false,1);
    ...
    public function getBbsSearch(){
        ...
        $result = TopicServiceT::getPostsList($fid,'','',$page,$pagesize,$subj,$sort,false,false,$isGood,$active,$uid,$tid,$search['startdate'],$search['enddate'],$display_order,$isTop,0,'true',false,false,$isAdmin);
//        $totalcount = count($result['result']);
        $totalcount = TopicServiceT::getPostsNum($fid,'',$uid,$tid,$isTop,$active,$isGood,'','',$sort,'',$search['startdate'],$search['enddate'],$subj,'true',false,false,$display_order,$isAdmin);
        if(!$result['errorCode'] && $result['result']) {
            ...
            $bbs_res = TopicServiceT::getForums('',1,10,false,1,$fids);
    ...
    public function postBbsAdd(){
        ...
        $result = TopicServiceT::doPostAdd($fid,$input['author_uid'],$bid,$input['subject'],$content,$coin,1,$disply_order,false,'true',
        ...
        if($result['errorCode']==0){
            $limit_res = TopicServiceT::add_replylimit(array('targetId'=>$result['result'],'targetType'=>'TOPIC','limitNum'=>$input['limit'],
                'limitDeadline'=>$input['limit_deadline'],'limitRate'=>$input['limit_rate'],'limitStatus'=>$input['limit_status'] ? 'true' : 'false'));
            return $this->redirect('/webt_forum/topic/bbs-search','发帖成功');
        }else{
    ...
    public function getBbsEdit($tid=''){
        if(!$tid) return Redirect::to('webt_forum/topic/bbs-search')->with('global_tips','数据错误');
        $topic_result = TopicServiceT::getPostDetail($tid);
        if($topic_result['errorCode'] || !$topic_result['result']) return Redirect::to('webt_forum/topic/bbs-search')->with('global_tips','无效帖子');
        $topic = $topic_result['result'];
        $uinfo = UserService::getUserInfoByUid($topic['uid']);
        $fid = $topic['fid'];
        if($fid){
            $forum_result = TopicServiceT::getForumDetail($fid);
        }else{
            $forum_result = array();
        }

        //if($forum_result['errorCode']) return Redirect::to('webt_forum/topic/bbs-search')->with('global_tips','无效帖子');
        ...
        $data['uinfo'] = $uinfo;
        //整理模块
        $board_result = TopicServiceT::getForumBoardList($topic_result['result']['fid']);
        $board_arr = array();
        ...
        $result=TopicServiceT::replylimitList($params);
...
//            'cid.required'=>'请选择论坛版块',
        ...
        $summary = "";$imgs=array();//暂时去掉
        $giftId = isset($input['giftId']) ? $input['giftId'] : '';
        $result = TopicServiceT::modifyTopic($tid,$input['author_uid'],$input['fid'],$bid,$input['subject'],$content,$coin,$input['message'],implode(',',$imgs),false,'true',
            'false','false',$is_top,$top_end_time,$reply_invisible,$disply_order,$summary,$giftId);
        if(!$result['errorCode']){
            if(!$input['limit_id']){
                //添加
                TopicServiceT::add_replylimit(array('targetId'=>$tid,'targetType'=>'TOPIC','limitNum'=>$input['limit'],
                'limitDeadline'=>$input['limit_deadline'],'limitRate'=>$input['limit_rate'],'limitStatus'=>$input['limit_status'] ? 'true' : 'false'));
            }else{
                //编辑
                TopicServiceT::edit_replylimit(array('id'=>$input['limit_id'],'targetId'=>$tid,'targetType'=>'TOPIC','limitNum'=>$input['limit'],
                    'limitDeadline'=>$input['limit_deadline'],'limitRate'=>$input['limit_rate'],'limitStatus'=>$input['limit_status'] ? 'true' : 'false',
...
//                print_r($input);die;
            ...
            return $this->redirect('/webt_forum/topic/bbs-search','修改成功');
        }else{
    ...
    public function getBbsSearchSelect(){
        ...
        $result=TopicServiceT::getForums($params['name'],$params['pageIndex'],$params['pageSize'],false,'1',false,false,1);
        $result=self::getdatainfo($result);
        foreach($result['result'] as $key=>$value){
            $datafid[]=!empty($value['fid'])?$value['fid']:'';
        }
        $resultinf=TopicServiceT::getForumAndGameRelation(1,1,$datafid);
        ...
        $count=TopicServiceT::getForumsCount($params['name'],false,1);
    ...
    public function getCheckGameHasBbs(){
        ...
        if($gameid && is_numeric($gameid)){
            $result = TopicServiceT::getForumAndGameRelation(1,1,false,$gameid);
            if($result['errorCode'] || !$result['result']) {
                $data['state'] = 0;
    ...
    public function getReplyList($tid='',$recycle=0){
        if(!$tid) return $this->redirect('/webt_forum/topic/search')->with('global_tips','数据错误');
        $topic_result = TopicServiceT::getPostDetail($tid);
        if($topic_result['errorCode'] || !$topic_result['result']) return Redirect::to('webt_forum/topic/bbs-search')->with('global_tips','无效帖子');
        $topic = $topic_result['result'];
        $uinfo = UserService::getUserInfoByUid($topic['uid']);
        ...
        $result = TopicServiceT::getReplyCommentList($tid,$page,$limit,1,'TOPIC',false,$is_active,false,'','','false',
            $keyword,$has_pic,$floor,$add_user);
        $total_result = TopicServiceT::getReplyTotalCount($tid,'',$is_active,false,'',null,null,null,$keyword,$floor,$has_pic);
    ...
    public function getCommentList($rid=''){
        if(!$rid) return $this->redirect('/webt_forum/topic/search')->with('global_tips','数据错误');
        $page = Input::get('page',1);
        $limit = 10;
        $vdata['comments'] = $uids = array();
        $result = TopicServiceT::getComments($rid,$page,$limit);
        $total_result = TopicServiceT::getCommentsCount($rid);
        $total = $total_result['errorCode'] ? 0 : $total_result['totalCount'];
        if(!$result['errorCode']){
    ...
    private function filterCommentData($comments,$uinfos){
        ...
        foreach($comments as $row){
            ...
            $result_comments[] = array(
                ...
                'content' => isset($row['isAdmin']) && $row['isAdmin'] ? $row['formatContent'] : (isset($row['content']) ? TopicServiceT::formatTopicMessage($row['content']) : ''),
...
//                'comments' => $row['comments'] ? true : false
            ...
            //查询二级恢复数量
            $total_result = TopicServiceT::getCommentsCount($row['id']);
    ...
    public function getReplyAdd($tid=''){
        ...
        if($rid&&!$uid){
            $result = TopicServiceT::getReplyDetail($rid);
...
//        print_r($input);die;
        if($input['uid']){
            if($input['id']){
                $res = TopicServiceT::update_reply($input);
            }else{
                $res = TopicServiceT::add_reply($input);
            }
        }else{
            if($input['id']){
                $input1['id'] = $input['id'];
                ...
                $res = TopicServiceT::update_reply($input1);
            }else{
                $res = TopicServiceT::add_reply($input);
            }
        }
        ...
        if($res && !$res['errorCode']){
            return $this->redirect('webt_forum/topic/reply-list/'.$input['tid'],'成功');
        }else{
            return $this->redirect('webt_forum/topic/reply-list/'.$input['tid'],$res['errorDescription']);
        }
    }

    public function getEditReply($rid=''){
        if(!$rid) return $this->back()->with('global_tips','数据错误');
        $result = TopicServiceT::getReplyCommentList('',1,1,0,'TOPIC','','true',false,$rid);
        if($result && !$result['errorCode']){
            $data = $result['result']['replys'][0];
            $data['content'] = isset($data['isAdmin']) && $data['isAdmin'] ? $data['formatContent'] : (isset($data['content']) ? TopicServiceT::formatTopicMessage($data['content']) : '');
    ...
    public function postEditReply(){
        ...
        $content = json_encode($content);
        $format_content = $txt;
        $result = TopicServiceT::updateReply($id,$input['replier_id'],$content,$format_content,implode(',',$imgs),'false',$input['tid']);
        if($result && !$result['errorCode']){
            return $this->redirect('webt_forum/topic/edit-reply/'.$id,'更新成功');
        }else{
    ...
    public function getDelReply($rid='',$uid=''){
        ...
        $result = TopicServiceT::delReply($rid,'true','false',$uid);
    ...
    public function getResReply($rid='',$uid=''){
        if(!$rid) return $this->back()->with('global_tips','数据错误');
        $result = TopicServiceT::delReply($rid,'true','true',$uid);
        if($result['errorCode']){
    ...
    public function getDelComment($cid='',$uid=''){
        ...
        $result = TopicServiceT::delComment($cid,$uid);
        if($result['errorCode']){
            $input = Input::get();
            $input['type'] = '1';
            $input['linkType'] = '5';
    ...
    public function getBbsDel($tid,$type='true',$back=true){
        $result = TopicServiceT::delTopic($tid,$type);
        if(!$result['errorCode']){
            $input = Input::get();
            //var_dump( $input);die();
            $input['type'] = '1';
        ...
        return $this->redirect('webt_forum/topic/bbs-search')->with('global_tips',$result['errorCode'] ? '操作失败' : '操作成功');
    }
    ...
    public function postBbsDel()
    {
        ...
        $result = TopicServiceT::delTopic($tids_str,$type);
    ...
    public function postBbsDigest()
    {
        ...
        $tid = Input::get('tid');
        if($act=='on'){
            $result = TopicServiceT::setTopicStatus($tid,false,'true');
            if(!$result['errorCode']){
                $input = Input::get();
                $input['type'] = '2016';
                $input['linkType'] = '2';
            ...
            $result = TopicServiceT::setTopicStatus($tid,false,'false');
        }
        return $this->json(array('status'=>$result['errorCode'] ? 0 : 1,'msg'=>$result['errorCode'] ? '操作失败' : '操作成功'));
    }
    ...
    public function postBbsStick()
    {
        ...
        if($act=='on'){
            $result = TopicServiceT::setTopicStatus($tid,'true');
            if(!$result['errorCode']){
                $input = Input::get();
                $input['type'] = '2016';
                $input['linkType'] = '1';
            ...
            $result = TopicServiceT::setTopicStatus($tid,'false');
        }
    ...
    public static function  system_send($input){
        ...
        $res = TopicServiceT::system_send($data);
        
        //系统推送
        $template = TopicServiceT::get_sys_mess_template(array('messageType'=>$input['type'].'_'.$input['linkType']));
        ...
        if ($template['result']) {
            ...
            $res = TopicServiceT::system_push($data);
        }
...
//礼包，商品发放消息推送
    ...
    public function getBbsSelect(){
        $keyword = Input::get('q');
        $bbs_res = TopicServiceT::getForums($keyword,1,10,false,1);
        $data = array();
        if(!$bbs_res['errorCode'] && $bbs_res['result']){
    ...
    public function getBbsInit(){
        $fid = Input::get('id');
        $bbs_res = TopicServiceT::getForums('',1,10,false,1,$fid);
    ...
    public function postBoardSearchSelect(){
        ...
        $board_result = TopicServiceT::getForumBoardList($bbs_id);
        if($board_result['errorCode'] || !$board_result['result']){
    ...
    public function getSetTopicPrize(){
        $data = array();
        $res = TopicServiceT::query_dictionary_list($data);
    ...
    public function postSetTopicPrize(){
        ...
        $res = TopicServiceT::update_dictionary($data);
        if(!$res['errorCode']&&$res['result']){
