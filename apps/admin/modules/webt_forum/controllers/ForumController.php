...
use modules\webt_forum\controllers\TopicController;
...
//        print_r($games);
        ...
        $forum_res = TopicServiceT::getForums($name,$page,$size,$game_id,1);
        ...
        $topic_num_res = TopicServiceT::getForumsCount($name,$game_id);
    ...
    public function getForumNumber()
    {
        ...
        $topic_num_res = TopicServiceT::getTopicNumber('','',$s_time_start,$s_time_end);
        if(!$topic_num_res['errorCode']) $data['topic_num'] = $topic_num_res['totalCount'];
        
        $reply_num_res = TopicServiceT::getReplyTotalCount(null,'',false,false,'',1,$s_time_start,$s_time_end);
    ...
    public function postAdd(){
        ...
        $add_res = TopicServiceT::addForum($input['forum_name'],$path);
        if($add_res['errorCode'] || !$add_res['result']) return $this->back()->withInput()->with('global_tips',$add_res['errorDescription']);
        $rel_res = TopicServiceT::add_game_link($add_res['result'],$input['gid'],self::GENRE);
        if(!$rel_res['errorCode'] && $rel_res['result']){
            return $this->redirect('webt_forum/forum/forum-list','添加成功');
        }else{
    ...
    public function getEdit($fid=''){
        ...
        $game_info = array();
        $info_res = TopicServiceT::getForumDetail($fid,self::GENRE);
        if($info_res['errorCode'] || !$info_res['result']) return $this->back('论坛不存在');
//        $rel_res = TopicServiceT::getForumAndGameRelation(1,self::GENRE, $fid );
//        print_r($info_res);
    ...
    public function postEdit(){
        ...
        $add_res = TopicServiceT::updateForum($input['fid'],$input['forum_name'],$path); //改成编辑，暂未提供
        if($add_res['errorCode'] || !$add_res['result']) return $this->back()->withInput()->with('global_tips','添加失败');
        $rel_res = TopicServiceT::add_game_link($input['fid'],$input['gid'],self::GENRE);
        if($add_res['errorCode']==0){
            return $this->redirect('webt_forum/forum/forum-list','修改成功');
        }else{
    ...
    public function getOpen($fid,$status,$gid="")
    {
    	...
    	$result = TopicServiceT::openForum($fid,$status,1);
    ...
    public function getAjaxDel($fid=''){
        ...
        $rel_res = TopicServiceT::getForumAndGameRelation(1,self::GENRE,$fid);
        if(!$rel_res['errorCode'] && $rel_res['result']){
            TopicServiceT::delForumAndGameRelation($fid,$rel_res['result'][0]['gid'],self::GENRE);
        }
        $result = TopicServiceT::deleteForum($fid);
    ...
    public function getForumBoardList($fid=''){
        if(!$fid) return $this->back('数据错误');
        $forum_res = TopicServiceT::getForumDetail($fid);
        if($forum_res['errorCode'] || !$forum_res['result']) return $this->back('论坛不存在');
        $result = TopicServiceT::getForumBoardList($fid);
    ...
    public function postAddBoard(){
        ...
        $add_res = TopicServiceT::addBoard($input['forum_name'],$type,$fid,$path);
        if(!$add_res['errorCode'] && $add_res['result']){
            return $this->redirect('webt_forum/forum/forum-board-list/'.$input['old_fid'],'添加成功');
        }else{
            return $this->redirect('webt_forum/forum/forum-board-list/'.$input['old_fid'],'添加失败');
        }
    }
    
    public function getEditBoard($bid,$fid)
    {
    	$board_res = TopicServiceT::getBoardDetail($bid);
    	if($board_res['errorCode']!=0) return $this->back('数据错误');
    	$board = $board_res['result'];
        if($board['fid']!=0){
            $forum_res = TopicServiceT::getForumDetail($board['fid']);
            $forum_info = $forum_res && !$forum_res['errorCode'] ? $forum_res['result'] : array();
        }else{
            $forum_info= array();
        }
    ...
    public function postEditBoard(){
        ...
        $add_res = TopicServiceT::updateBoard($input['bid'],$input['forum_name'],$type,$fid,$path);
        if($add_res['errorCode']==0){
            return $this->redirect('webt_forum/forum/forum-board-list/'.$input['old_fid'],'添加成功');
        }else{
            return $this->redirect('webt_forum/forum/forum-board-list/'.$input['old_fid'],'添加失败');
    ...
    public function getAjaxOpenCloseBoard(){
        ...
        $res = TopicServiceT::openCloseBoard($bid,$is_open);
    ...
    public function getMasterApplicationList(){
        ...
        $page = Input::get('page',1);
        $size = 10;
        $res = TopicServiceT::getMasterpplicationAList($page,$size);
        ...
        foreach($result as $k=>$v){
            ...
            $arr_user[] = $v['proposer'];
            //查询申请人是否是版主
            $master = TopicServiceT::getWebMaster('',$v['targetId'],$v['targetType'],$v['proposer']);
            ...
            $president = 0;
            $vice_president = 0;
            $master = TopicServiceT::getWebMaster('',$v['targetId'],$v['targetType']);
            if(!$master['errorCode'] && $master['result']){
...
//        $arr_user = array_flip($arr_user);
        ...
        $board_res = TopicServiceT:: getBoardList('','',$str_board);
        $forum_res = TopicServiceT:: getForums('',1,10,false,false,$str_forum);
        //可以封装一下
...
//                $result[$k]['gid'] = $arr_forum_gid[$v['targetId']];
    ...
    public function getSetMaster($id,$targetId,$targetType,$uid,$isDeputy){
        ...
        $res = TopicServiceT::SetMaster($id,$targetId,$targetType,$uid,$isDeputy);
        $data = array(
            'uid' =>$uid,
            'delete' => 0,
        ...
        if(!$res['errorCode'] && isset($res['result'])){
            $res2 = TopicServiceT::grant_user_medal($data);
            $input = Input::get();
            $input['type'] = '2015';
            $input['linkType'] = '1';
            //$input['link'] = $targetId;
           ...
           //var_dump($input);die();
            ...
            return $this->redirect('webt_forum/forum/master-application-list','操作成功');
        }else{
    ...
    public function getDelMaster($id,$uid){
        ...
        $res = TopicServiceT::DelMaster($id,$uid);
        if(!$res['errorCode'] && isset($res['result'])){
            $res2 = TopicServiceT::grant_user_medal($data);
            if($res['result'] == "1"){
                return $this->redirect('webt_forum/forum/master-application-list','该用户在其他论坛任然是版主！');
            }
            return $this->redirect('webt_forum/forum/master-application-list','操作成功');
        }else{
            return $this->back()->withInput()->with('global_tips','操作失败');
    ...
    public function getRefuseApplication($id){
        $res = TopicServiceT::RefuseApplication($id);
        if(!$res['errorCode'] && $res['result']){
            return $this->redirect('webt_forum/forum/master-application-list','操作成功');
        }else{
            return $this->back()->withInput()->with('global_tips','操作失败');
    ...
    public function getRecruitRuleList(){
        ...
        $totle = TopicServiceT::recruit_rule_list(array('pageSize'=>"999"));
        $res = TopicServiceT::recruit_rule_list($search);
        if(!$res['errorCode'] && $res['result']){     
            foreach($res['result'] as $k=>$v){
                if($v['targetId']  && $v['targetId']!="all"){
                    $bbs = TopicServiceT::getForumDetail($v['targetId']);
...
//            unset($input['targetType']);
            $res = TopicServiceT::update_recruit_rule($input);
        }else{
            $res = TopicServiceT::add_recruit_rule(array_filter($input));
        }
...
//        print_r($res);die;
    ...
    public function postDelRecruitRule(){
        $input = input::get();
        $res = TopicServiceT::delete_recruit_rule($input);
//        print_r($input);
//        print_r($res);die;
        if(!$res['errorCode']&&$res['result']){
            echo json_encode(array('success'=>"true",'mess'=>'修改成功','data'=>""));
    ...
    function postSetHot(){
        ...
        if($type == "1"){
            ...
            $data['place'] = "0";
            $res = TopicServiceT::add_recommend($data);
        }else{
            $res = TopicServiceT::del_recommend(array('recommendId'=>input::get('id')));
        }
        if(!$res['errorCode']){
            echo json_encode(array('success'=>"true",'mess'=>'修改成功','data'=>""));
        }else{
    ...
    public function getHotForumList(){
        ...
        $forum_res=TopicServiceT::forum_recommend_list($search);

        if(!$forum_res['errorCode'] && $forum_res['result']) $vdata['list'] = $forum_res['result'];
        $topic_num_res = TopicServiceT::getForumsCount($name);
        if(!$topic_num_res['errorCode']) $vdata['totalcount'] = $topic_num_res['totalCount'];
    ...
    public function  getBbsRelationGame($fid="",$game='',$gid='')
    {
        ...
        $result=TopicServiceT::query_game_link_list(true,2,$fid);
    ...
    public function postBbsGameRelation(){
        ...
        $result=TopicServiceT::query_game_link_list(true,Input::get('type'),Input::get('fid'),Input::get('gid'));
        if(empty($result['result'])){
            $result=TopicServiceT::add_game_link(Input::get('fid'),Input::get('gid'),Input::get('type'));//获取type为平台
            if(!$result['errorCode']){
                return $this->redirect('webt_forum/forum/forum-list')->with('global_tips','关联成功');
            }else{
    ...
    public function getDelForumGame($fid=0,$gid=0,$type='')
    {
        ...
        $gid = Input::get('gid');
        $result=TopicServiceT::del_game_link($linkId);
        if(!$result['errorCode']){
            if($gid){
                GameModel::updateGameInfo($gid,array('is_open_forum'=> 0));
            }
            return $this->redirect('webt_forum/forum/forum-list')->with('global_tips','操作成功');
        }else{
            return $this->back()->with('global_tips','操作失败');
        }
    ...
    public function getCheckGameRelationBbs(){
        ...
        $result=TopicServiceT::getForumAndGameRelation(true,Input::get('type'),'',Input::get('gid'));
        if($result['errorCode'] != 0){
            $data['state'] = 0;
            $data['msg'] = '请求失败';
            return response::json($data);
